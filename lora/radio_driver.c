...
{
    //logPrintf("SUBGRF_GetOperatingMode\n\r");
    return OperatingMode;
}

void SUBGRF_SetPayload( uint8_t *payload, uint8_t size )
{
    //logPrintf("SUBGRF_SetPayload\n\r");
    SUBGRF_WriteBuffer( 0x00, payload, size );
}

uint8_t SUBGRF_GetPayload( uint8_t *buffer, uint8_t *size,  uint8_t maxSize )
{
    //logPrintf("SUBGRF_GetPayload\n\r");
    uint8_t offset = 0;
...
{
    //logPrintf("SUBGRF_SetCrcPolynomial\n\r");
    uint8_t buf[2];
...
{
    //logPrintf("SUBGRF_SetWhiteningSeed\n\r");
    uint8_t regValue = 0;
...
uint32_t SUBGRF_GetRandom( void )
{
    ...
    ////logPrintf( "SUBGRF_GetRandomEnd()\n\r");
    return number;
}

void SUBGRF_SetSleep( SleepParams_t sleepConfig )
{
    //logPrintf( "SUBGRF_SetSleep\n\r");
...
{
    ////logPrintf( "\n\r");
	//logPrintf( "SUBGRF_SetRxDutyCycle(rxTime=%d,sleepTime=%d)\n\r",rxTime,sleepTime);
    uint8_t buf[6];
...
{
    //logPrintf( "SUBGRF_SetTxContinuousWave\n\r");
    SUBGRF_WriteCommand( RADIO_SET_TXCONTINUOUSWAVE, 0, 0 );
}

void SUBGRF_SetTxInfinitePreamble( void )
{
    //logPrintf( "SUBGRF_SetTxInfinitePreamble\n\r");
    SUBGRF_WriteCommand( RADIO_SET_TXCONTINUOUSPREAMBLE, 0, 0 );
}

void SUBGRF_SetStopRxTimerOnPreambleDetect( bool enable )
{
    //logPrintf( "SUBGRF_SetStopRxTimerOnPreambleDetect\n\r");
    SUBGRF_WriteCommand( RADIO_SET_STOPRXTIMERONPREAMBLE, ( uint8_t* )&enable, 1 );
}

void SUBGRF_SetLoRaSymbNumTimeout( uint8_t symbNum )
{
    //logPrintf( "SUBGRF_SetLoRaSymbNumTimeout\n\r");
...
{
    //logPrintf( "SUBGRF_SetRegulatorMode\n\r");
...
{
    //logPrintf( "SUBGRF_CalibrateImage\n\r");
    uint8_t calFreq[2];
...
{
    //logPrintf( "SUBGRF_SetPaConfig\n\r");
    uint8_t buf[4];
    ////logPrintf( "SUBGRF_SetPaConfig(paDutyCycle=%d,hpMax=%d,tcxoVoltage=%d,timeout=%d)\n\r",paDutyCycle,hpMax,deviceSel,paLut);
    buf[0] = paDutyCycle;
    buf[1] = hpMax;
    buf[2] = deviceSel;
    buf[3] = paLut;
...
{
    //logPrintf( "SUBGRF_SetRxTxFallbackMode\n\r");
    SUBGRF_WriteCommand( RADIO_SET_TXFALLBACKMODE, &fallbackMode, 1 );
}

void SUBGRF_SetDioIrqParams( uint16_t irqMask, uint16_t dio1Mask, uint16_t dio2Mask, uint16_t dio3Mask )
{
    //logPrintf( "SUBGRF_SetDioIrqParams\n\r");
    uint8_t buf[8];
...
{
    //logPrintf( "SUBGRF_SetTcxoMode\n\r");
	////logPrintf( "SUBGRF_SetTcxoMode(tcxoVoltage=%d,timeout=%d)\n\r",tcxoVoltage,timeout);
    uint8_t buf[4];
...
{
	//logPrintf( "SUBGRF_SetRfFrequency(%d)\n\r",frequency);
    uint8_t buf[4];
    uint32_t chan = 0;
...
{
    //logPrintf( "SUBGRF_SetPacketType\n\r");
...
{
    //logPrintf( "SUBGRF_GetPacketType\n\r");
    return PacketType;
}

void SUBGRF_SetTxParams( uint8_t paSelect, int8_t power, RadioRampTimes_t rampTime )
{
	//logPrintf( "SUBGRF_SetTxParams(%d,%d,0x%02x)\n\r",paSelect,power,(int8_t)rampTime);
    uint8_t buf[2];
    int32_t max_power;
...
{
	//logPrintf( "SUBGRF_SetModulationParams()\n\r");
    uint8_t n;
    uint32_t tempVal = 0;
...
{
	//logPrintf( "SUBGRF_SetPacketParams()\n\r");
    uint8_t n;
    uint8_t crcVal = 0;
...
{
    //logPrintf( "SUBGRF_SetBufferBaseAddress\n\r");
    uint8_t buf[2];
...
{
    //logPrintf( "SUBGRF_GetRxBufferStatus\n\r");
    uint8_t status[2];
...
{
    //logPrintf( "SUBGRF_GetPacketStatus\n\r");
    uint8_t status[3];
    ...
    switch( pktStatus->packetType )
    {
        ...
        case PACKET_TYPE_NONE:
        	...
        	UTIL_MEM_set_8( pktStatus, 0, sizeof( PacketStatus_t ) );
...
{
    //logPrintf( "SUBGRF_GetDeviceErrors\n\r");
    uint8_t err[] = { 0, 0 };
...
{
    //logPrintf( "SUBGRF_ClearDeviceErrors\n\r");
...
{
    //logPrintf( "SUBGRF_ClearIrqStatus\n\r");
    uint8_t buf[2];
...
{
    //logPrintf( "SUBGRF_WriteRegister\n\r");
	//////logPrintf( "SUBGRF_WriteRegister(0x%x,0x%02x)\n\r",addr,data);
    HAL_SUBGHZ_WriteRegisters( &hsubghz, addr, (uint8_t*)&data, 1 );
}

uint8_t SUBGRF_ReadRegister( uint16_t addr )
{
    //logPrintf( "SUBGRF_ReadRegister\n\r");
    uint8_t data;
...
{
    //logPrintf( "SUBGRF_WriteRegisters\n\r");
    CRITICAL_SECTION_BEGIN();
    HAL_SUBGHZ_WriteRegisters( &hsubghz, address, buffer, size );
    ////logBuffer("SUBGRF_WriteRegisters",address, buffer, size );
    CRITICAL_SECTION_END();
}

void SUBGRF_ReadRegisters( uint16_t address, uint8_t *buffer, uint16_t size )
{
    //logPrintf( "SUBGRF_ReadRegisters\n\r");
    CRITICAL_SECTION_BEGIN();
    HAL_SUBGHZ_ReadRegisters( &hsubghz, address, buffer, size );
    //logBuffer("SUBGRF_ReadRegisters",address, buffer, size );
    CRITICAL_SECTION_END();
}

void SUBGRF_WriteBuffer( uint8_t offset, uint8_t *buffer, uint8_t size )
{
	//logBuffer("SUBGRF_WriteBuffer",offset, buffer, size );
    CRITICAL_SECTION_BEGIN();
...
void SUBGRF_ReadBuffer( uint8_t offset, uint8_t *buffer, uint8_t size )
{
    ...
    CRITICAL_SECTION_END();
    //logBuffer("SUBGRF_ReadBuffer",offset, buffer, size );
}

void SUBGRF_WriteCommand( SUBGHZ_RadioSetCmd_t Command, uint8_t *pBuffer,
                                        uint16_t Size )
{
	//logBuffer("SUBGRF_WriteCommand",Command, pBuffer, Size );
    CRITICAL_SECTION_BEGIN();
                                        ...
                                        uint16_t Size )
{
    ...
    CRITICAL_SECTION_END();
    //logBuffer("SUBGRF_ReadCommand",Command, pBuffer, Size );
}

void SUBGRF_SetSwitch( uint8_t paSelect, RFState_t rxtx )
{
	//logPrintf( "SUBGRF_SetSwitch(0x%x,0x%02x)\n\r",paSelect,(uint8_t)rxtx);
...
{
	//logPrintf( "SUBGRF_SetRfTxPower(0x%x)\n\r",power);
...
{
	//logPrintf( "SUBGRF_GetRadioWakeUpTime()\n\r");
...
{
	//logPrintf( "HAL_SUBGHZ_RxTxTimeoutCallback()\n\r");
    RadioOnDioIrqCb( IRQ_RX_TX_TIMEOUT );
}

void HAL_SUBGHZ_HeaderErrorCallback(SUBGHZ_HandleTypeDef *hsubghz)
{
	//logPrintf( "HAL_SUBGHZ_HeaderErrorCallback()\n\r");
    RadioOnDioIrqCb( IRQ_HEADER_ERROR );
}

void HAL_SUBGHZ_PreambleDetectedCallback(SUBGHZ_HandleTypeDef *hsubghz)
{
	//logPrintf( "HAL_SUBGHZ_PreambleDetectedCallback()\n\r");
    RadioOnDioIrqCb( IRQ_PREAMBLE_DETECTED );
}

void HAL_SUBGHZ_SyncWordValidCallback(SUBGHZ_HandleTypeDef *hsubghz)
{
	//logPrintf( "HAL_SUBGHZ_SyncWordValidCallback()\n\r");
    RadioOnDioIrqCb( IRQ_SYNCWORD_VALID );
}

void HAL_SUBGHZ_HeaderValidCallback(SUBGHZ_HandleTypeDef *hsubghz)
{
	//logPrintf( "HAL_SUBGHZ_HeaderValidCallback()\n\r");
    RadioOnDioIrqCb( IRQ_HEADER_VALID );
}

static void Radio_SMPS_Set(uint8_t level)
{
	//logPrintf( "Radio_SMPS_Set(%d)\n\r",level);
  if ( 1U == RBI_IsDCDC() )
  {
    uint8_t modReg;
...
{
    //logPrintf( "SUBGRF_GetFskBandwidthRegValue\n\r");
    uint8_t i;
